...
var ea = new function() {
    ...
    this.reset = function() {
        ...
        f = 1 - p.h * p.h * p.h * .01;
        l = -p.i * p.i * p.i * 1E-6;
        p.a || (n = .5 - p.n / 2, w = 5E-5 * -p.o);
        r = 1 + p.l * p.l * (0 < p.l ? -.9 : 10);
        g = 0;
    ...
    this.K = function(p, u) {
        var e = this.A,
            ...
            $a = 1 / c,
            ab = 1 / b;
        e = 5 / (1 + e.u * e.u * 20) * (.01 + B);
        .8 < e && (e = .8);
        e = 1 - e;
        for (var oa = !1, Da = 0, U = 0, V = 0, Ea = 0, ba = 0, W, X = 0, y, J = 0, K, pa = 0, t, Fa = 0, O, Ga = 0, ca = Array(1024), Y = Array(32), C = ca.length; C--;) ca[C] = 0;
        ...
        for (C = 0; C < u; C++) {
            ...
            y |= 0;
            8 > y && (y = 8);
            ma || (n += w, 0 > n ? n = 0 : .5 < n && (n = .5));
            if (++U > na) switch (U = 0, ++Da) {
                case 1:
                    na = c;
                    break;
                case 2:
            ...
            za && (Aa += Wa, K = Aa | 0, 0 > K ? K = -K : 1023 < K && (K = 1023));
            I && F && (E *= F, 1E-5 > E ? E = 1E-5 : .1 < E && (E = .1));
            O = 0;
            for (var bb = 8; bb--;) {
                J++;
                if (J >= y && (J %= y, 3 == ma))
                ...
                switch (ma) {
                    ...
                    case 2:
                        t =
                            J / y;
                        t = 6.28318531 * (.5 < t ? t - 1 : t);
                        t = 1.27323954 * t + .405284735 * t * t * (0 > t ? 1 : -1);
                        t = .225 * ((0 > t ? -1 : 1) * t * t - t) + t;
                        break;
                    case 3:
                        t = Y[Math.abs(32 * J / y | 0)]
                }
                I && (W = X, B *= N, 0 > B ? B = 0 : .1 < B && (B = .1), la ? (ba += (t - X) * B, ba *= e) : (X = t, ba = 0), X += ba, Ea += X - W, t = Ea *= 1 - E);
...
window.jsfxr = function(a) {
    ...
    for (var d = "data:audio/wav;base64,"; b < c; b += 3) {
        ...
        d += "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/" [k >> 18] + "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/" [k >>
            12 & 63
        ] + "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/" [k >> 6 & 63] + "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/" [k & 63]
    }
    return d
};
const fa = a => {
    ...
    a.G.map((u, e) => {
        ...
        for (let F = 0; F < u.length; F += 4) {
            ...
            let N = c.get(B);
            if (void 0 === N) {
                I[5] = .1 * Math.sqrt(55E4 * Math.pow(2, (u[F + 2] - 69) / 12) / a.D - .1);
                const la = jsfxr(I);
                N = {
                    next: 0,
                    H: Array(a.C).fill(null).map(() => {
                        const T = new Audio;
                        d++;
                        let aa = !1;
    ...
    const w = () => {
        ...
        p = () => {
            ...
            if (r) g = !0;
                ...
                var e = new Date,
                    I = Math.min((e - l) / 1E3, .1);
                l = e;
                if (u)
                    for (k += I; k >= b[f].time;)
                        if (u = b[f++], e = u.B.H[u.B.next], u.B.next = (u.B.next + 1) % a.C, 0 < n && (void 0 === document.hidden || !document.hidden) && (e.volume = u.volume * n, e.play()), f === b.length)
                            if (h) w();
                            else {
                                g = !0;
                                return
...
const m = (a, c, b) => {
    ...
    ia = [fa({
        ...
        D: 44100,
        C: 5,
        F: [null, null, [0, .1, .1, null, .3, .6, null, -.1, null, null, null, null, null, null, null, null, null, null, 1, null, null, null, null, .5], null, null, [0, .1, .1, null, .8, .6, null, -.1, null, null, null, null, null, null, null, null, null, null, 1, null, null, null, null, .2], null, null, null, null, null, null, null, null, null,
            null, null
        ],
        G: [
            [],
            [],
            [3, 1, 48, 51, 0, 1, 55, 51, 1, 1, 55, 51, 0, 1, 63, 51, 1, 1, 63, 51, 0, 1, 72, 51, 1, 1, 67, 51, 0, 1, 55, 51, 2, 1, 67, 51, 0, 1, 75, 51, 1, 1, 72, 51, 0, 1, 63, 51, 1, 1, 48, 51, 0, 1, 55, 51, 1, 1, 55, 51, 0, 1, 63, 51, 2, 1, 63, 51, 0, 1, 72, 51, 1, 1, 67, 51, 0, 1, 55, 51, 1, 1, 67, 51, 0, 1, 75, 51, 1, 1, 72, 51, 0, 1, 63, 51, 2, 1, 48, 51, 0, 1, 55, 51, 1, 1, 55, 51, 0, 1, 63, 51, 1, 1, 63, 51, 0, 1, 72, 51, 1, 1, 67, 51, 0, 1, 55, 51, 2, 1, 67, 51, 0, 1, 75, 51, 1, 1, 72, 51, 0, 1, 63, 51, 1, 1, 48, 51, 0, 1, 55, 51, 1, 1, 55, 51, 0, 1, 63, 51, 2, 1, 63, 51, 0, 1, 72, 51, 1, 1, 67, 51, 0, 1, 55, 51, 1, 1, 67, 51, 0, 1, 75, 51, 1, 1, 72, 51, 0, 1, 63, 51,
                ...
                51, 0, 1, 43, 51, 2, 1, 74, 51, 0, 1, 65, 51, 1, 1, 71, 51, 0, 1, 55, 51, 1, 1, 55, 51, 0, 1, 74, 51, 0, 1, 79, 51, 1, 1, 50, 51, 0, 1, 59, 51, 0, 1, 74, 51, 2, 1, 55, 51, 0, 1, 65, 51, 1, 1, 62, 51, 0, 1, 43, 51, 1, 1, 74, 51, 0, 1, 65, 51, 1, 1, 71, 51, 0, 1, 55, 51
            ],
            [],
            [],
            [3, 7, 48, 101, 0, 7, 60, 85, 0, 7, 72, 70, 0, 7, 36, 117, 12, 3, 55, 101, 0, 3, 43, 101, 3, 7, 48, 101, 0, 7, 60, 85, 0, 7, 72, 70, 15, 7, 43, 101, 0, 7, 55, 85, 0, 7, 67, 70, 0, 7, 31, 79, 30, 7, 48, 101, 0, 7, 60, 85, 0, 7, 72, 70, 0, 7, 36, 117, 15, 7, 48, 101, 0, 7, 60, 85, 0, 7, 72, 70, 0, 7, 36, 117, 15, 7, 48, 101, 0, 7, 60, 85, 0, 7, 72, 70, 0, 7, 36, 117, 15, 7, 43, 101, 3, 4, 55, 95, 4, 7, 67, 85, 4,
                4, 79, 79, 4, 7, 91, 71, 7, 7, 103, 63
            ],
            [],
            [],
            [],
        ...
        C: 5,
        F: [
            [0, null, .1, null, .3, .6, null, -.3, null, null, null, null, null, null, null, null, null, null, 1, null, null, null, null, .5], null, [0, null, .1, null, .3, .6, null, -.1, null, null, null, null, null, null, null, null, null, null, 1, null, null, null, null, .4], null, null, null, null, null, null, null, null, null, null, null, null, null, null
        ],
        G: [
            [1536, 319, 69, 161, 320, 63, 64, 161, 64, 255, 68, 161, 256, 63, 69, 161, 64, 63, 68, 161, 64, 319, 67, 161, 320, 63, 62, 161, 64, 191, 66, 161, 192,
                47, 64, 161, 48, 47, 66, 161, 48, 47, 67, 161, 48, 47, 68, 161, 48, 255, 69, 161, 256, 63, 69, 161, 64, 63, 64, 161, 64, 255, 68, 161, 256, 63, 69, 161, 64, 63, 68, 161, 64, 255, 67, 161, 256, 63, 67, 161, 64, 63, 62, 161, 64, 191, 66, 161, 192, 47, 64, 161, 48, 47, 66, 161, 48, 47, 67, 161, 48, 47, 68, 161, 48, 255, 69, 161, 256, 63, 69, 161, 64, 63, 64, 161, 64, 255, 68, 161, 256, 63, 69, 161, 64, 63, 68, 161, 64, 255, 67, 161, 256, 63, 67, 161, 64, 63, 62, 161, 64, 47, 64, 161
            ],
            [],
            [0, 47, 57, 161, 48, 47, 64, 161, 48, 47, 65, 161, 48, 47, 64, 161, 48, 47, 72, 161, 48, 47, 64, 161, 48, 47, 65, 161, 48, 47, 64, 161, 48, 47, 57, 161, 48, 47,
...
const ra = (a, c = null) => {
        ka = c || ka;
        q === a && ja || (null !== q && qa(), q = a, a.J(ka), ha || (a.play(!0), ja = !0))
    },
    qa = (a = !0) => {
        null !== q && (q.stop(), q = a ? null : q, ja = !1)
    },
    sa = [() => ra(ia[0], .7), () => ra(ia[1], .4), () => qa(), m([3, .47, .23, , .69, .53, , -.18, -.04, , , , , , , , , , 1, , , , , .29], 1, .75), m([0, .49, .33, , .36, .16, , .1, , , , , , , , , , , 1, , , , , .3], 1, .75), m([3, , .09, , .3, .74, , -.5, , , , , , , , , , , 1, , , .22, , .3], 1, .75), m([3, , .2, .6, .06, .1, , .13, , , , , , , , , , , 1, , , , , .29], 1, .75), m([3, , .23, , .17, .4, , -.4, , , , , , , , , , , 1, , , , , .29], 1, .75), m([0, , .03, , .3, .2, , .2, , , , , , .44, , , , , 1, , , , , .29], 1, .75), m([3, 1.5, .31, , .89, .5, , -.12, , , , , , , , , , , 1, , , , , .29], 1, .75), m([3, , .2, .5, .34, .13, , .2, , , , , , , , , , , 1, , , , , .29], 1, .75), m([3, , .11, , .2, .42, , -.32, .1, , , , , , , , , , 1, , , , , .29], 1, .75), m([2, .29, .51, .43, .67, .35, , -.14, , .33, .4, , , .4, , , , , 1, , , , , .29], 3, .75), m([2, , .11, , .47, .6, , -.44, , , , , , , , , , , 1, , , .09, , .29], 3, .75), m([3, .9, .23, , .69, .53, , , -.04, , , , , , , , , , 1, , , , , .5], 1, .2), m([3, , .24, .13, .78, .1, , -.2217, , , , , , , , .7, .27, -.15, 1, , , , , .29], 1, .2)],
    v = [64, 7, 128, 128, 128, 130, 130, 0, 1, 161, 0, 13, 124, 111, 126, 110, 130, 110, 132, 111, 134, 114, 132, 114,
        ...
        11, 12, 4, 124, 124, 132, 124, 132, 126, 124, 126, 0, 0, 0
    ],
...
const Ka = [2, 0, 2, 7, 7, 7, 0, 0, 0],
    La = () => {
        ...
        z = A.getContext("2d");
        Ia = ["#4b0072", "#9800e5", "#000", D(2, 2, (a, c) => (a + c) % 2 ? "#cf0d67" : "#000"), D(1, 2, (a, c) => c % 2 ? "transparent" : "#454a98"), D(1, 2, (a, c) => c % 2 ? "transparent" : "#864598"), D(1, 2, (a, c) => c % 2 ? "transparent" : "#458a98"), "#cf0d67", "#000"];
        Ja = Ia.map((a, c) => Ka[c] ? D(2, 2, (b, d) => (b + d) % 2 ? a : "transparent") : "transparent")
    };
(() => {
    let a = 0;
    va = [...Array(v[a++])].map(() => [...Array(v[a++])].map(() => [].concat([...Array(5)].map(() => v[a++] - 128), 0, 0, [...Array(4)].map(() => v[a++]), [
...
const Ma = a => {
    ...
    Na = a => {
        M += a;
        a = Math.floor(M / .05) + (0 < a);
        for (xa = Math.abs(M - .05 * a) / .05; wa !== a;) {
            const c = 2 * (wa < a) - 1;
            wa += c;
            L.map(b => {
                ...
                if (!ua(b)) {
                    var k = b[9] & 31;
                    b[6] = 2 === k ? .1 * d[0] : 3 === k ? .1 * d[1] : b[6] + .05 * c;
                    b[12].map(f => {
                        const l = f[1].reduce((h, n) => h + .1 * n[0], 0);
                        let r = b[6 + 13 * (0 > c)];
                        b[9] & 128 && (r = (r + 1E8 * l) % l);
                        const g = f[1].length;
                        x(g).map(h => {
                            if (h !== g - 1) {
                                var n =
                                    f[1][h];
                                h = f[1][(h + 1) % g];
                                if (0 <= r && r < .1 * n[0] + 1E-10 && 0 < n[0]) {
                                    var w = r / n[0] / .1;
                                    h = n[1] * (1 - w) + h[1] * w;
                                    w = f[0];
                                    if (6 === w % 7) {
                                        if (w = (new Date).getTime(), 0 < h && (!f[2] || w > f[2] + 1500) && (f[2] = w, h = k - 16, 0 <= h && h < sa.length)) sa[h]()
                                    } else b[w % 7] = 6 < w ? h : b[w] + .05 * h * c
                                }
                                r -= .1 * n[0]
                            }
                        })
    ...
    Pa = (a, c, b, d) => {
        ...
        const k = (0 < (a[9] & 64)) / 2;
        z.beginPath();
        3 > a[11].length ? (a = a[11][0], z.arc(a[0], a[1], 4, 0, 2 * Math.PI)) : a[11].map((f, l) => {
            var r = f[0];
...
let Q, R = 1,
    ...
    cb = 0;
const db = a => {
        let c = Math.min((a - cb) / 1E3, .02);
        cb = a;
        c *= 0 > (Ua -= c);
        requestAnimationFrame(db);
        Ta *= .95;
        ya = .9 * ya + (Math.random() - .5) * Ta;
        Ha = .9 * Ha + (Math.random() - .5) * Ta;
        Na(-.05);
        Na(c * R + .05);
        const b = R;
        R = 1;
        ...
        if (void 0 !== Q) {
            ...
            var d = [-(P.has(37) || P.has(65)) + (P.has(39) || P.has(68)), -(P.has(38) || P.has(87)) + (P.has(40) || P.has(83))].map(g => 18 * g * c),
            ...
            if (0 > Ua)
                for (d = 0; 9 > d; d++) {
                    ...
                    var l = z.getImageData(8 * Q[0] + 320 + f, 8 * Q[1] + 184 + g, 1, 1).data,
                        r = !l[1] && !l[0];
                    if (l[1] && !l[0]) {
                        l = L[l[1] - 1];
                        const h = l[8],
                        ...
                        2 === h || 3 === h ? r = !0 : 4 === h ? (R = -1, a = 1) : 5 === h ? (R = 0, a = 2) : 6 === h && (R = +(0 < k), a = 3);
                        10 === n ? Q[1] = -1E3 : 11 === n ? G.add(l[10]) : 12 === n ? G.delete(l[10]) : 13 === n && H.add(l[10])
                    }
                    r && 8 > d && (r = Math.sqrt(f ** 2 + g ** 2), Q[0] -= f / r * c * 19.8, Q[1] -= g / r * c * 19.8)
                }
            k = S;
            d = Z;
            S += -(-38 > Q[0]) + (38 < Q[0]);
            Z += -(-21 > Q[1]) + (21 < Q[1]);
            if (k !== S || d !== Z) eb(k < S ? -36 : k > S ? 36 : Q[0], d < Z ? -19 : d > Z ? 19 : Q[1]), ya += 640 * (k - S), Ha += 368 * (d - Z), Ua = .7
        }
        z.fillStyle = Ia[2];
    ...
    fb = () => {
        ...
        let c = 0,
            ...
            d = innerWidth / 640;
        innerWidth / innerHeight > a && (c = (innerWidth - innerHeight * a) / 2, b = 0, d = innerHeight / 368);
    ...
    eb = (a, c) => {
        ...
        Ra = () => {
            const d = 8 * S + Z;
            0 > d || 63 < d || (G = new Set(b), H.clear(), Ma(32 ===
                d ? [d] : [d, 0]), Q = L.find(k => 1 === (k[9] & 31)), void 0 !== Q && (Q[0] = a, Q[1] = c))
        };
        Ra()
    };
addEventListener("load", () => {
    La();
    Sa = [0, 250, 300, 200].map(a => D(100, 100, (c, b) => `hsla(${a}, 100%, ${10*(0<a)}%, ${.1+b%2*.2+.2*Math.random()})`));
