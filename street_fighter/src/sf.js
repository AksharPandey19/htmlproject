(function() {
  ...
  mk.fighters.Fighter.prototype.endureAttack = function(damage, attackType) {
    ...
    if (this.getLife() === 0) {
      ...
      let name = this._name.charAt(0).toUpperCase() + this._name.slice(1).toLowerCase();

      // If the name is not "Kano" and its length is more than 3, add a hyphen after the third letter
      if (name !== "Kano" && name.length > 3) {
          name = name.slice(0, 3) + '-' + name.slice(3);
      }

      document.getElementsByClassName("gend")[0].textContent = name + "  died.";
    }
    return this.getLife();
