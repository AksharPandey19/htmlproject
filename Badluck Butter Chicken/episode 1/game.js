// Waring⚠️ Writing code is 10% typing, 90% googling errors. 

const log = (t) => console.log(t); // If this line makes it to production, we riot! 🚨

// Math Variables (a.k.a. The Nerd Squad)
const M = Math;
const min = M.min; // Because we love keeping things minimal
const abs = M.abs; // For those moments when we can't handle negativity
const cos = M.cos; // The "cool" side of trigonometry
const sin = M.sin; // The "sinful" side of trigonometry
const PI2 = M.PI * 2; // Because one π isn’t enough for us 🍕

// Screen Dimension (a.k.a. The Window to Our Soul)
const WIDTH = 1920; // Big enough for all the chaos
const HEIGHT = 1080; // Because 1079 just wasn’t good enough

const px = 'px' // Pixels, because inches are too mainstream

// Sound Effect IDs (a.k.a. The Noisy Gang)
const FX_CLICK = 0; // Clickety-click
const FX_COIN = 1; // Money talks 💰
const FX_DEFLECT = 2; // Boing! That was close...
const FX_SHIELD = 3; // You shall not pass! 🛡️
const FX_MAGNET = 4; // Come to me, little ones 🧲
const FX_BESTSCORE = 5; // Flex mode: ON
const FX_ALARM = 6; // PANIC! 🚨

// Keyboard Variables (a.k.a. The Chaos Controllers)
let keysEnabled; // Are the keys even listening?
let waitingForKey; // 1 if the app is waiting for a new control key to be pressed (Cue dramatic music)
let leftHeld; // Is the player permanently stuck moving left? Find out next episode!
let rightHeld; // Or maybe they're just running away from responsibility
let controlIndex; // The index of the control key to be changed (because change is hard)
let controlLabel; // The poor UI element waiting to be updated
let CONTROL_RIGHT = 1; // Because going right is always right... right?
let CONTROL_LEFT = 0; // Or maybe left is right? Wait, what?

// Game modes (a.k.a. The Mood Swings of the Game)
const GAME_MODE_MENUS = 0; // When the player is just hanging out in menus.
const GAME_MODE_PLAYING = 1; // When things actually get serious.
const GAME_MODE_GAMEOVER = 2; // When regret kicks in. 😭

// Game timing variables (because time is an illusion)
let DT; // The ever-elusive delta time.
let elapsedTime = 0; // Time flies when you're having fun (or debugging).
let thisFrame; // "This" frame, not to be confused with "that" frame.
let lastFrame; // The frame that just couldn't keep up.
let paused; // Is the game taking a break?
let pauseState; // Because pausing isn’t as simple as it sounds.

// UI (User Inconvenience?)
let activeMenu; // The current menu that players are trapped in.
let cursorVisible = 1; // 1 = cursor exists, 0 = it's playing hide-and-seek.
let gamemode; // What mode are we even in right now?

// Should it be let or const? That is the question.
let MULTIPLIER = 1; // Multiplier for velocities, making things more difficult over time. (Because why make life easy?)

// Score management (a.k.a. The Ego System)
let playerScore; // The number that determines happiness.
let gotBestScore; // Did we just make history?
let spawningBestScoreEffects; // If we got a best score, it better come with fireworks!
let bestScoreEffectsSpawnTimer; // How long until the celebration starts?
let bestScoreEffectsSpawnCounter; // How many effects have we spawned? (Too many is never enough)

// Graphical assets (a.k.a. The Pretty Stuff)
let textureRegions = []; // Where all the art magic happens.
let svgString; // A bunch of text that somehow turns into an image. 🧙
let SVG_ID = 0; // Because every SVG needs a unique identity. 

// Background clouds that refuse to go away.
let clouds = [];
const CLOUD_COUNT = 31; // A suspiciously specific number.
const FRONT_CLOUD_COUNT = 5; // The VIP clouds.
const MIDDLE_CLOUD_COUNT = 8; // The not-so-VIP clouds.
const cloudPointData = [ // Each cloud point has an x, y, and a radius (Cloud physics 101)
  748, 379, 50, 
  680, 402, 50, 
  718, 96, 50, 
  94, 502, 50, 
...
// Coin management (Money, money, money 💰)
let coinTrailTimer; // Coins don’t just appear... or do they?
let coinSpawnTimer; // When is the next payday?
let coinPool; // A pool of coins, just waiting for their moment.
let activeCoins; // The ones that made it into the game. 

let frames = 10; // Number of frames. Why 10? No one knows.
let duration = 0.5; // Duration in seconds (half a second of fame).

// Badluck Butter Chicken (A.k.a. The Player)
let player;
const playerAcceleration = 150; // Because speed matters.
const playerMaxVelocity = 1200; // Go big or go home.
const playerLeanFactor = PI2 * 0.005; // Tilt your head slightly when reading this.

// Pickups (A.k.a. Power-ups and Excuses)
let shieldOverlay; // For those "I totally dodged that" moments.

let pickupPool; // Where all the goodies wait.
let activePickups; // The chosen ones.

let shielded; // If true, you are invincible... for now.
let shieldCounter; // How long until reality hits?
let shieldPickupSpawnTimer; // When’s the next free shield?

let magnetized; // Are we pulling in coins like a boss?
let magnetCounter; // How long will this madness last?
let magnetPickupSpawnTimer; // When’s the next magnet?
let magnetEffectSpawnTimer; // How often do we show the magnetic vibes?

// Other game entities (a.k.a. The Things That Ruin Your Day)
let enemies; // Because life isn't fair.

let collisionsEnabled; // Should we let players suffer the consequences of bad decisions?

let particles; // Small, insignificant things that make the game look cooler.

let renderList; // A VIP list of things that actually get drawn on screen.

// Actor Types (Hollywood Edition 🎬)

/**
 * NOTE: These values are also used for z-sorting 
 * (a fancy way of saying "who appears in front of whom?")
 */

const ACTOR_TYPE_FARCLOUD = 0; // The clouds that don’t like socializing.
const ACTOR_TYPE_SUN = 1; // THE SUN WAS NOT A PAID ACTOR. ☀️
const ACTOR_TYPE_NEARCLOUD = 2; // These clouds are nosy.
const ACTOR_TYPE_COIN = 3; // The real reason we play the game.
const ACTOR_TYPE_TOKEN = 4; // Because just having coins wasn’t enough.
const ACTOR_TYPE_MAGNET_PICKUP = 5; // Attracts coins AND attention.
const ACTOR_TYPE_SHIELD_PICKUP = 6; // Blocks everything, including bad decisions.

const ACTOR_TYPE_ENEMY = 8; // The villain of every game.
const ACTOR_TYPE_PLAYER = 9; // That’s you! No pressure.
const ACTOR_TYPE_SHIELD = 10; // The barrier between you and disaster.
const ACTOR_TYPE_PARTICLE = 11; // Little bits of chaos.

// TODO: Actually learn WebGL instead of copy-pasting code from 10 different sources.
...
// I have no idea how shaders work. If this works, it's a miracle.  
...
// Welcome to the world's most sophisticated sound system. Also known as: "random noises that somehow work."
...
const menuMusicModule = [
  ...
  [ // Patterns.
    [
      ...
      [3,1,29,29.88,29,29.88,29,25.88,29,29.88,29,29.88,29,29.88,29,25.88,29,29.88,29,29.88,29,29.88,29,25.88,29,29.88,29,29.88,29,25.88,29,29.88,25,25.88,29,29.88,29,29.88,29,25.88,29,29.88,29,29.88,29,29.88,29,25.88,29,29.88,29,29.88,29,25.88,29,29.88,25,29.88,29,29.88,29,25.88,29,29.88,29,29.88], // This line of code is here to confuse future developers. It’s working.  
  ...
  ], // This music module is the digital equivalent of a cat walking on a synthesizer.
  [1,0,0,2], // Sequence.
  150 // BPM.
];
...
// If this works, I'm a genius. If it doesn't, I'm blaming JavaScript.  
...
const gameOver = e => {
  ...
  setHTML(finalScoreLabel, `You scored ${playerScore.toLocaleString()} points (${['play Roblox KIDDO!', 'Pagalpanti', 'Pathetic', 'Terrible', 'Poor', 'Mediocre', 'Fair', 'Modest', 'Adequate', 'Good', 'Solid', 'Excellent', 'Superb', 'Fantastic', 'Stupendous'][min((playerScore / 1e4) | 0, 14)]})`);
...
// Uncomment the next line to unleash chaos.  
// gl2_gl.destroyEverything();
};